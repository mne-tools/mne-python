...
def make_grid_layout(info=None, shape=None, width=4, height=5):
    ...
    info : dict
        ...
        employed.
    shape : tuple | None
        row * cols. If None, shape will be infered from measurement info
    Returns
    -------
    layout : Layout
        The generated layout.
    ...
    """
    if not any([info, shape]):
        raise ValueError('Please supply measurement info, size or shape!')
        ...
        ids = range(len(inds))
        size = len(inds)
    elif info is None and shape is not None:
        size = np.prod(*shape)
        ids = range(size)
        names = ['X %3d' % i for i in xrange(size)]
    ...
    # calculate pos
    pos = np.c_[x, y, width * np.ones(size), height * np.ones(size)]

    # calculate box
    box = (x.min() - 0.1 * width, x.max() + 1.1 * width,
           y.min() - 0.1 * width, y.max() + 1.1 * height)

    layout = Layout(box=box, pos=pos, names=names, kind='Grid-custom', ids=ids)
    return layout
