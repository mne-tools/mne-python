...
from mne.io import (Raw, concatenate_raws,
                    get_chpi_positions, set_eeg_reference, specify_eeg_montage)
...
def test_specify_eeg_montage():
    """ Test specifying various EEG montages"""
    raw = Raw(fif_fname, preload=True)
    ...
    reref, ref = specify_eeg_montage(raw, ref=['EEG 001', 'EEG 002'], copy=True)
    ...
    reref, ref = specify_eeg_montage(raw, ref=[], copy=True)
    assert_array_equal(np.mean(raw[picks_eeg][0], axis=0), ref)

    # Test using the EOG channel as reference
    reref, ref = specify_eeg_montage(raw, ref=['EOG 061'], copy=True)
    assert_array_equal(reref[picks_eeg][0] + reref[picks_eog][0], raw[picks_eeg][0])

    # Test constructing bipolar channels
    bipolar1 = picks_eeg[:2]
    bipolar2 = picks_eeg[2:4]
    reref, ref = specify_eeg_montage(raw,
        bipolar={'BIPOLAR1': bipolar1, 'BIPOLAR2': bipolar2}, copy=True)
    ...
    reref, ref = specify_eeg_montage(raw, ref=['EEG 005', 'EEG 006'],
    ...
    # Test that data is modified in place when copy=False
    reref, ref = specify_eeg_montage(raw, ref=['EEG 001', 'EEG 002'],
        copy=False)
    assert_true(raw is reref)
