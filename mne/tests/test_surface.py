...
def test_warp_montage_volume():
    ...
    for i, d in enumerate(montage_warped.dig):
        assert (
            np.linalg.norm(d["r"] - ground_truth_warped[i])  # off by less than 1.5 cm
            < 0.015
        )
    # check image_from
    ...
    for i in range(len(montage.ch_names)):
        assert (
            np.linalg.norm(
                np.array(np.where(np.array(image_to.dataobj) == i + 1)).mean(axis=1)
                - ground_truth_warped_voxels[i]
            )
            < 8
        )
    # test inputs
    with pytest.raises(ValueError, match="`thresh` must be between 0 and 1"):
        with pytest.warns(FutureWarning, match="deprecated"):
            warp_montage_volume(
                montage, CT, reg_affine, sdr_morph, "sample", thresh=11.0
            )
