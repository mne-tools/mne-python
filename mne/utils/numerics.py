...
def _time_mask(
    ...
    if raise_error and not mask.any():
        ...
        raise ValueError(
            f"No samples remain when using tmin={orig_tmin}"
            f"and tmax={orig_tmax} {extra}"
...
def _freq_mask(freqs, sfreq, fmin=None, fmax=None, raise_error=True):
    ...
    if raise_error and not mask.any():
        raise ValueError(
            ...
            f"fmax={orig_fmax} (original frequency bounds are"
            f"[{freqs[0]}, {freqs[-1]}])"
        )
    return mask
...
def _check_dt(dt):
    if (
        ...
        raise ValueError(f"Date must be datetime object in UTC: {dt}")
