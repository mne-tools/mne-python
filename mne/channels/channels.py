...
def set_channel_types_eyetrack(inst, mapping):
    """Define the sensor type for eyetrack channels with all specific
    information.
    ...
    # allowed
    allowed_types = ['eyetrack_pos', 'eyetrack_pupil']  # ch_type
    allowed_units = {'px': ['px', 'pixel'],
                     'deg': ['deg', 'degree', 'degrees'],
                     'mm': ['mm', 'diameter'],
                     'au': [None, 'area']}
    allowed_eye = {'l': ['left', 'l'],
                   'r': ['right', 'r']}
    allowed_xy = {'x': ['x', 'h', 'horizontal'],
                  'y': ['y', 'v', 'vertical']}
    ...
    for ch_name, ch_desc in mapping.items():
        ...
        if ch_type not in allowed_types:
            raise ValueError(
                ...
                "Got '{}' instead.".format(allowed_types, ch_type))
        ...
        if ch_unit not in [item for sublist in allowed_units.values()
            ...
            raise ValueError(
                "unit must be one of {}. "
                "Got '{}' instead.".format(
                    [item for sublist in allowed_units.values()
                     for item in sublist], ch_unit))
        if ch_unit in allowed_units['px']:
            unit_new = FIFF.FIFF_UNIT_PX
        elif ch_unit in allowed_units['deg']:
            unit_new = FIFF.FIFF_UNIT_DEG
        elif ch_unit in allowed_units['mm']:
            unit_new = FIFF.FIFF_UNIT_MM
        elif ch_unit in allowed_units['au']:
        ...
        if ch_eye not in [item for sublist in allowed_eye.values()
                          for item in sublist]:
            raise ValueError(
                "eye must be one of {}. "
                "Got '{}' instead.".format(
                    [item for sublist in allowed_eye.values()
                     for item in sublist], ch_eye))
        if ch_eye in ['left', 'l']:
            loc[3] = -1
        elif ch_eye in ['right', 'r']:
            loc[3] = 1

        if ch_type == 'eyetrack_pos':
            ch_xy = ch_desc[3].lower()
            if ch_xy not in [item for sublist in allowed_xy.values()
                             for item in sublist]:
                raise ValueError(
                    "x/y must be one of {}. "
                    "Got '{}' instead.".format(
                        [item for sublist in allowed_xy.values()
                         for item in sublist], ch_xy))
            if ch_xy == 'x':
                loc[4] = -1
            elif ch_xy == 'y':
                loc[4] = 1
