...
if [ ! -z "$CONDA_ENV" ]; then
	...
	pip install $STD_ARGS --pre --only-binary ":all:" --no-deps  --default-timeout=60 -i "https://pypi.anaconda.org/scipy-wheels-nightly/simple" numpy scipy
	pip install $STD_ARGS --only-binary ":all:" --no-deps  --default-timeout=60 scikit-learn dipy
