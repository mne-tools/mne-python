...
import numpy as np
from numpy.linalg import pinv, svd, multi_dot, lstsq
...
def _fixed_phase1b(gain,s_ap,data_cov,attr_dict):
    ...
    for _ in range(1,attr_dict['nsources']):
        ...
        for dip in range(attr_dict['ndipoles']):
            l_p          = np.expand_dims(gain[:,dip], axis=1)
            ap_val2[dip] = multi_dot([l_p.transpose(),perpend_spc,data_cov,perpend_spc,l_p])\
                /((multi_dot([l_p.transpose(),perpend_spc,l_p]))[0,0])
...
def _calculate_fixed_alternating_projections(data_arr, gain, nsources, max_iter):
    """
...
def _solve_active_gain_eig(ind,gain,data_cov,eig,perpend_spc):
    ...
    l_p  = gain[:,gain_idx]
    eig_a = multi_dot([l_p.transpose(),perpend_spc,data_cov,perpend_spc,l_p])
    eig_b = multi_dot([l_p.transpose(),perpend_spc,perpend_spc,l_p])
...
def _free_phase1a(attr_dict,gain,data_cov):
    ...
    for dip in range(attr_dict['ndipoles']):
        sol_tuple = _solve_active_gain_eig(dip,gain,data_cov,eig,perpend_spc)
    ...
    sol_tuple = _solve_active_gain_eig(s1_idx,gain,data_cov,eig,perpend_spc)
...
def _free_phase1b(attr_dict,gain,data_cov,ap_temp_tuple):
    ...
    for src in range(1,attr_dict['nsources']):
        ...
        s2_idx = np.argmax(ap_val2)
        s_ap.append(s2_idx)
        sol_tuple = _solve_active_gain_eig(s2_idx,gain,data_cov,eig,perpend_spc)
        oris[src] = sol_tuple[1][:,[np.argmax([x.real for x in sol_tuple[0]])]][:,0]
        sub_g_proj = np.concatenate([sub_g_proj,np.dot(sol_tuple[2],oris[src])[:,np.newaxis]],axis=1)
    return s_ap, oris, sub_g_proj
...
def _free_phase2(ap_temp_tuple,attr_dict,data_cov,gain):
    ...
    for itr in range(attr_dict['max_iter']):
        ...
        for src in range(attr_dict['nsources']):
            ...
            s_ap_2[src] = sq_idx
            sol_tuple = _solve_active_gain_eig(sq_idx,gain,data_cov,eig,perpend_spc)
            oris[src] = sol_tuple[1][:,[np.argmax([x.real for x in sol_tuple[0]])]][:,0]
...
def _calculate_free_alternating_projections(data_arr, gain, nsources, max_iter):
    """
...
def free_ori_ap(wh_data,gain,nsources,forward,max_iter):
    ...
    sol_tuple = \
        _calculate_free_alternating_projections(wh_data, gain, nsources, max_iter)
...
def fixed_ori_ap(wh_data,gain,nsources,forward,max_iter):
    ...
    idx = _calculate_fixed_alternating_projections(wh_data,gain,nsources=nsources,
                                                     max_iter=max_iter)
...
def _explained_data_packing(evoked,picks,explained_data_mat,info):
    ...
    active_projs = [proj for proj in explained_data.info['projs'] if proj['active']]
    for proj in active_projs:
        proj['active'] = False
...
def ap(evoked,forward,nsources,noise_cov=None,max_iter=6,
    ...
    return_active_info : bool, optional
        If True, appends estimated source's information (indices,coordinates,orientation).
    ...
    output : list
        ...
        if return_active_info :
            idx : list of int
                List of indices of dipole source estimated.
            poss : array, shape (nsources, 3)
                Coordinates array of estimated sources (sorted by idx appearence).
            oris : array, shape (nsources, 3)
                Orientations array of estimated sources (sorted by idx appearence).
    ...
    if return_residual:
        ...
        #treating residual
        residual = _residual_packing(evoked,picks,explained_data_mat,info)

        #treating explained data
        info     = evoked.info
        explained_data = _explained_data_packing(evoked,picks,explained_data_mat,info)
