...
info = evoked.info
helmet_surf = mne.get_meg_helmet_surf(info, trans)

# these need to be in head coordinates
head_surf = mne.get_head_surface('sample')

head_map = mne.make_surface_mapping(info, head_surf, trans, 'eeg', n_jobs=0)
helmet_map = mne.make_surface_mapping(info, helmet_surf, trans, 'meg',
                                      n_jobs=0)

# let's look at the N100
evoked.crop(0.100, 0.110)
...
for ii, (surf, data) in enumerate(zip([head_surf, helmet_surf],
    ...
    mesh = mlab.pipeline.triangular_mesh_source(x, y, z, surf['tris'],
                                                scalars=data)
    surf = mlab.pipeline.surface(mesh, opacity=alphas[ii])
    surf.module_manager.scalar_lut_manager.lut.table = colormap
    surf = mlab.pipeline.contour_surface(mesh, contours=40, line_width=1.0)
    surf.module_manager.scalar_lut_manager.lut.table = colormap
