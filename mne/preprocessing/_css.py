...
def temp_proj(ref_data_2, ref_data_1, raw_data, n_proj=6):
    """
    This temporal projection procedure removes the common signal subspace 
    between ref_data_2 and ref_data_1 from raw_data using n_proj number of 
    projection vectors. Normally used for cortical signal suppression, where
    ref_data_1 is gradiometer data, ref_data_2 is magnetometer data and 
    raw_data is EEG data.

    Parameters
    ----------
    ref_data_2 : np.ndarray of float, shape (n_sensors_2, n_times)
    ...
    ref_data_1 : np.ndarray of float, shape (n_sensors_1, n_times)
    ...
    Qg = linalg.qr(ref_data_1.T)[0]
    Qm = linalg.qr(ref_data_2.T)[0]
...
def cortical_signal_suppression(evoked, n_proj=6, show_psd=False):
    ...
    eeg_subcortical = temp_proj(mag_data, grad_data, eeg_data,
                           n_proj=n_proj)
