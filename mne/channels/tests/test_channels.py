...
def test_set_montage():
    ...
    EXPECTED_UNIT = [ch['unit'] for ch in EXPECTED_CHS]
    EXPECTED_COIL_TYPE = [ch['coil_type'] for ch in EXPECTED_CHS]
    ...
    actual_unit = [ch['unit'] for ch in raw.info['chs']]
    actual_coil_type = [ch['coil_type'] for ch in raw.info['chs']]
    actual_coord_ref = [ch['coord_frame'] for ch in raw.info['chs']]

    assert_array_equal(actual_names, EXPECTED_NAME)
    assert_array_equal(actual_loc, EXPECTED_LOC)
    assert_array_equal(actual_kind, EXPECTED_KIND)
    assert_array_equal(actual_unit, EXPECTED_UNIT)
    assert_array_equal(actual_coil_type, EXPECTED_COIL_TYPE)
