...
from mne.mixed_norm.inverse import mixed_norm, tf_mixed_norm
...
lh_label = 'Aud-lh'
fname_lh_label = op.join(data_path, 'MEG', 'sample', 'labels',
                         '%s.label' % lh_label)
rh_label = 'Aud-rh'
fname_rh_label = op.join(data_path, 'MEG', 'sample', 'labels',
                         '%s.label' % rh_label)
...
lh_label = read_label(fname_lh_label)
rh_label = read_label(fname_rh_label)
...
def test_tf_mxne_inverse():
    ...
    assert_true(stc.vertno[1][1] in rh_label['vertices'])
