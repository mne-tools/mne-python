    ...
    for i_ch, ch_name in enumerate(info.ch_names):
        ...
        if type(channel_dict) == dict:
            if ch_name not in channel_dict.keys():
                ...
                ch_type = channel_dict[ch_name]['ch_type'] \
                    if 'ch_type' in keys else ch_type
                ch_unit = channel_dict[ch_name]['ch_unit'] \
                    if 'ch_unit' in keys else ch_unit
                ch_eye = channel_dict[ch_name]['ch_eye'] \
                    if 'ch_eye' in keys else ch_eye
