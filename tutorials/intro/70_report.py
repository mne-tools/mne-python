...
# disk) and the ``title``, there is a third required paramer, ``inst``.
...
#
# In the following example, we crop the raw data, only fit ICA on EEG channels,
# request a small number of ICA components to estimate, limit the number of
# ICA iterations to a very small amount, and only visualize 2 of the
# components. All of this is done to largely reduce the processing time of this
# tutorial, and is usually **not** recommended for an actual data analysis.
...
ica = mne.preprocessing.ICA(
    n_components=5,  # fit 5 ICA components – this is too litle in practice!
    max_iter=100  # abort after 100 iterations – don't do this in pracice!
)
