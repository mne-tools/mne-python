...
def _proj_equal(a, b, check_active=True):
    ...
    equal = (
        (a["active"] == b["active"] or not check_active)
        and a["kind"] == b["kind"]
        and a["desc"] == b["desc"]
...
def _read_proj(fid, node, *, ch_names_mapping=None, verbose=None):
    ...
    for item in items:
        ...
        one = Projection(
            ...
            desc=desc,
            data=dict(
                nrow=nvec, ncol=nchan, row_names=None, col_names=names, data=data
            ),
    ...
    for k, p in enumerate(projs):
        if not p["active"] or include_active:
            ...
            for v in range(p["data"]["nrow"]):
                ...
                if psize > 0:
                    ...
                    if (
                        ...
                        and not inplace
                        and (
                            p["kind"] != FIFF.FIFFV_PROJ_ITEM_EEG_AVREF
                            or len(vecsel) == 1
                        )
                    ):
                        warn(
            ...
            if inplace:
                ...
                p["data"]["col_names"] = [p["data"]["col_names"][ii] for ii in vecsel]
...
def _normalize_proj(info):
    ...
    _make_projector(
        info["projs"],
        info.get("ch_names", info.get("names")),
        info["bads"],
        include_active=True,
        inplace=True,
    )
...
def _has_eeg_average_ref_proj(
    ...
    want_names = [
        info["ch_names"][pick]
        for pick in pick_types(info, exclude="bads", **pick_kwargs)
    ]
    if not want_names:
        return False
    found_names = list()
    ...
    if missing:
        if found_names:  # found some but not all: warn
            warn("Incomplete %s projector, missing channel(s) %s" % (ch_type, missing))
        return False
    return True
