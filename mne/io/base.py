...
class BaseRaw(
    ...
    def __repr__(self):  # noqa: D105
        ...
        s = (
            f"{name}{len(self.ch_names)} x {self.n_times} ({self.times[-1]:0.1f} s), "
            f"~{size_str}"
        )
...
def _write_raw_data(
    ...
    for first, last in zip(firsts, lasts):
        ...
        if overage > 0:
            ...
            raise ValueError(
                f"buffer size ({this_buff_size_bytes}) is too large"
                f" for the given split size ({split_size}) "
                f"by {overage} bytes after writing info ({pos_prev}) and"
                f" leaving enough space "
                f"for end tags ({_NEXT_FILE_BUFFER}): decrease"
                f' "buffer_size_sec" or increase '
                f'"split_size".'
            )
