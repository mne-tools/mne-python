...
For example, to create the reconstruction geometry for ``sample``
with a ~5-mm spacing between the grid points, say::

    >>> src = setup_source_space('sample', 'sample-oct6-src.fif', spacing='oct6')
...
.. note:: The :ref:`mne_convert_surface` C utility can be used to convert text format    triangulation files into the FreeSurfer surface format.
...
This processing stage is done using :func:`mne.setup_bem_model` as::

    >>> model = make_bem_model('sample')
    >>> write_bem_surfaces('sample-5120-5120-5120-bem.fif', model)

Where ``surfaces`` is a list of surfaces that have each been read using
:func:`mne.read_surface`.

Using this model, the BEM solution can be computed as::
...
BEM model meshes are correctly positioned using :class:`mne.Report`.
...
As explained in :ref:`BJEBIBAI`, the head coordinate
...
slightly user dependent. As a result, the it is safest to reestablish
...
.. warning:: This step is extremely important. If the alignment of the
...
This computation stage is facilitated :func:`mne.make_inverse_operator` as::

    >>> inv = mne.make_inverse_operator(raw.info, fwd, cov, loose=0.2)
