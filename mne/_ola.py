    ...
    def feed(self, *datas, **kwargs):
        ...
        for di, data in enumerate(datas):
            ...
            if self._in_buffers[di] is None:
                ...
                self._in_buffers[di] = np.empty((data.shape[0], 0), data.dtype)
            if data.ndim != 2 or self._in_buffers[di].dtype != data.dtype:
                raise TypeError('data must be 2D %s with shape[0]==%d, '
                                'got %s shape %s'
                                % (self._in_buffers[di].dtype,
                                   self._in_buffers[di].shape[0],
        ...
        while self._idx < len(self.starts) and \
            ...
            if self._out_buffers is None:
                ...
                self._out_buffers = [np.zeros((len(o), max_len),
...
class _Storer(object):
    ...
    def __init__(self, *outs, **kwargs):
        ...
        self.picks = slice(None) if picks is None else picks

    def __call__(self, *outs):
        if (len(outs) != len(self.outs) or
                not all(out.ndim == 2 for o in outs for out in outs) or
                not all(out.shape[1] == outs[0].shape[1] for out in outs)):
            raise ValueError('Bad outs')
        for o1, o2 in zip(self.outs, outs):
            o1[self.picks, self.idx:self.idx + o2.shape[1]] = o2
        self.idx += outs[0].shape[1]
