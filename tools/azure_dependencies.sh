...
if [ "${TEST_MODE}" == "pip" ]; then
	...
	echo "Numpy etc."
	python -m pip install $STD_ARGS --only-binary ":all:" --default-timeout=60 --extra-index-url "https://pypi.anaconda.org/scientific-python-nightly-wheels/simple" "numpy<2.0.0.dev0" "scipy==1.12.0" "scikit-learn>=1.5.dev0" matplotlib pillow statsmodels pyarrow h5py
	# echo "dipy"
