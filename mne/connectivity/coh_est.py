...
@verbose
def _coherency_est(data, idx=None, sfreq=2*np.pi, fmin=0, fmax=np.inf,
                   bandwidth=None, adaptive=False, low_bias=True,
                   print_name='coherency', verbose=None):
    ...
    logger.info('%s computation...' % print_name.capitalize())
    for epoch_idx, data_i in enumerate(data):
        ...
        if epoch_idx == 0:
            ...
            logger.info('    computing %s for %d connections'
                        % (print_name, n_con))
        ...
        logger.info('    computing %s for epoch %d'
                    % (print_name, epoch_idx + 1))
    ...
    logger.info('[%s computation done]' % print_name.capitalize())
...
def coherency(data, idx=None, sfreq=2*np.pi, fmin=0, fmax=np.inf,
    ...
    name = 'coherency'
    coh, freqs, n_epochs, n_tapers = _coherency_est(data, idx=idx,
        sfreq=sfreq, fmin=fmin, fmax=fmax, bandwidth=bandwidth,
        adaptive=adaptive, low_bias=low_bias, print_name=name,
        verbose=verbose)
...
def imag_coherence(data, idx=None, sfreq=2*np.pi, fmin=0, fmax=np.inf,
              bandwidth=None, adaptive=False, low_bias=True,
              verbose=None):

    """Compute imaginary coherence between signals using a multi-taper method.

    The computed imaginary coherence is given by

    .. math:: C_{XY}(f) = \frac{IM(S_{XY}(f))}
                               {\sqrt{S_{XX}(f) S_{YY}(f)}}
    ...
    # the imaginary coherence is simply the imaginary part of the coherency
    coh = np.imag(coh)
