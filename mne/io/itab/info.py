...
from ..._fiff._digitization import DigPoint
...
def _mhdch_2_chs(mhd_ch):
    ...
    # Other channel type
    if (
        mhd_ch["type"] == ITAB.ITABV_REF_EEG_CH
        and mhd_ch["type"] == ITAB.ITABV_REF_MAG_CH
        and mhd_ch["type"] == ITAB.ITABV_REF_AUX_CH
        and mhd_ch["type"] == ITAB.ITABV_REF_PARAM_CH
        and mhd_ch["type"] == ITAB.ITABV_REF_DIGIT_CH
        and mhd_ch["type"] == ITAB.ITABV_REF_FLAG_CH
    ):
        ch["kind"] = FIFF.FIFFV_BIO_CH
        ch["cal"] = float(mhd_ch["amvbit"] / mhd_ch["calib"])
        ch["unit"] = FIFF.FIFF_UNIT_V
...
def _mhd2info(mhd):
    ...
    for i, point in enumerate(mhd["marker"]):
        ...
        point_info["r"] = np.array([point["posx"], point["posy"], point["posz"]])
    ...
    for sample in mhd["sample"]:
        event.append([sample["start"], sample["type"], sample["quality"]])
