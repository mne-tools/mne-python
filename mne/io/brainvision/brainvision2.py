...
for Brainvision Header Version 2.0"""

# Authors: Teon Brooks <teon.brooks@gmail.com>
#          Christian Brodbeck <christianbrodbeck@nyu.edu>
# Modified for Brain Vision Header version 2.0 format:
#          Marian Dovgialo <marian.dovgialo@fuw.edu.pl>
#
    ...
    def set_brainvision_events(self, events):
        ...
        if had_events and not has_events:  # remove stim channel
            ...
            chan_info = {'ch_name': 'STI 014',
                         ...
                         'eeg_loc': np.zeros(3), #DO IT ADD LOCATIONS
                         'loc': np.zeros(12)}
            self.info['nchan'] += 1
...
def _read_vmrk_events(fname):
    ...
    for info in items:
        ...
        if mtype == 'Pulse Artifact':
            onset = int(onset)
            duration = int(duration)
            events.append((onset, duration, 9999999))
        elif mtype == 'Bad Interval':
            onset = int(onset)
            duration = int(duration)
            events.append((onset, duration, 9999998))
...
def _get_eeg_info(vhdr_fname, eog, misc):
    ...
    with open(vhdr_fname, 'r') as f:
        ...
        l = f.readline().strip()
        assert l == 'Brain Vision Data Exchange Header File Version 2.0'
        settings = f.read()
...
def read_raw_brainvision2(vhdr_fname, montage=None,
