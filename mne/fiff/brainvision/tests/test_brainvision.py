...
from mne.fiff.brainvision import read_raw_eeg

FILE = inspect.getfile(inspect.currentframe())
data_dir = op.join(op.dirname(op.abspath(FILE)), 'data')
hdr_path = op.join(data_dir, 'test.vhdr')
...
def test_eeg_data():
    ...
    """
    raw_py = read_raw_eeg(hdr_path, elp=elp_path, preload=True)
...
def test_read_segment():
    ...
    """
    raw1 = read_raw_eeg(hdr_path, elp=elp_path, preload=False)
    ...
    raw2 = read_raw_eeg(hdr_path, elp=elp_path, preload=True)
