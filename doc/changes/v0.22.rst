...
Enhancements
~~~~~~~~~~~~
- Add :func:`mne.read_evokeds_mff` to read averaged MFFs (requires mffpy >= 0.5.7) by :newcontrib:`Evan Hathaway` (:gh:`8354`)

- Add :class:`mne.decoding.SSD` for spatial filtering with spatio-spectral-decomposition (:gh:`7070` by :newcontrib:`Victoria Peterson` and `Denis Engemann`_)

- Add reader for optical imaging data recorded using ISS Imgagent I/II hardware and BOXY recording software in :func:`mne.io.read_raw_boxy` (:gh:`7717` by :newcontrib:`Jonathan Kuziek` and `Kyle Mathewson`_)
...
Bugs
~~~~
- Fix orthogonalization of power envelopes in ``mne.connectivity.envelope_correlation`` (:gh:`8658` by :newcontrib:`Qianliang Li` and `Eric Larson`_)

- Fix data overwrite of cascading simulation operations :`mne.simulation.simulate_raw` (:gh:`8633` by :newcontrib:`Jeff Stout`)

- Fix a transpose issue of :func:`mne.decoding.CSP.plot_filters` (:gh:`8580` by :newcontrib:`Hongjiang Ye`)

- Fix :func:`mne.io.read_raw_curry` to deal with Curry datasets that have channels that are listed in the labels file, but which are absent from the saved data file (e.g. 'Ref' channel).  Also now populates info['meas_date'] if possible (:gh:`8400` by :newcontrib:`Tod Flak`)

- Fix bug with mne.io.egi.tests/test_egi.py where it mandatorily downloaded testing data when it was not necessary (:gh:`8474` by :newcontrib:`Aniket Pradhan`)

- Fix bug with reading split files that have dashes in the filename (:gh:`8339` by :newcontrib:`Eduard Ort`)

- Fix bug with parsing EDF dates and date integers (:gh:`8558` by :newcontrib:`Austin Hurst` and `Eric Larson`_)
