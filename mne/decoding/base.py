...
def _check_scoring(estimator, scoring=None, allow_none=False):
    ...
    Notes
    -----
    This fixes sklearn bug for `roc_auc` scoring.
    """
    from sklearn.metrics.scorer import check_scoring, make_scorer
    from sklearn.metrics import roc_auc_score

    if isinstance(scoring, str) and scoring == 'roc_auc':
        # XXX sklearn < 0.18 has a bug
        scoring = make_scorer(roc_auc_score, needs_threshold=False)
    else:
        scoring = check_scoring(estimator, scoring, allow_none)
    return scoring
