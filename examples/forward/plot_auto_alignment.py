...
data_path = mne.datasets.sample.data_path(path='/export/research/analysis/human/jhouck/shared/tools/MNE-sample-data')
subjects_dir = data_path + '/subjects'
subject='sample'
fname_raw = data_path + '/MEG/sample/sample_audvis_raw.fif'
fname_surf = subjects_dir + '/' + subject + '/bem/watershed/' + subject +  '_outer_skin_surface'
fname_tal = subjects_dir + '/' + subject + '/mri/transforms/talairach.xfm'
fname_fids = subjects_dir + '/' + subject + '/bem/' + subject + '-fiducials.fif'
fname_trans = data_path + '/MEG/sample/sample_audvis_raw_auto-trans.fif'
...
model.nasion_weight=2. # For the initial fit we know the nasion is not precise
# And overweighting at this step it seems to throw off the fit for some datasets
model.fit_icp()
model.omit_hsp_points(distance=5./1000) # note that this distance is in meters
# Do final coreg fit
...
print('Median distance for %s using ICP is %.3f mm' % (subject, np.median(errs_icp)*1000)) 
print('Median distance for %s using nearest neighbor is %.3f mm' % (subject, np.median(errs_nearest)*1000)) 
...
print('Median distance from digitized points to head surface is %.3f mm' % np.median(errs_icp)*1000) 
print('Median distance from digitized points to head surface using nearest neighbor is %.3f mm' % np.median(errs_nearest)*1000)
