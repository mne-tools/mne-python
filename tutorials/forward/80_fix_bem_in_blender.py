...
#
# (1) Select the surface that is creating the problem. (2) In *Edit Mode*, press
# ``C`` to use the circle selection tool to select the vertices that are
# outside.(3-5) In the *Object Data Properties* tab use the ``+`` button to add a
# *Vertex Group* and click *Assign* to assign the current selection to the group.
# (6-8) In the *Modifiers* tab go to *Add Modifier* add a *Shrinkwrap* modifier
# and set it to snap *Inside* with the outer surface as the *Target* and the
# *Group* that you created before as the *Vertex Group*. You can then use the
# *Offset* parameter to adjust the distance. (9) In *Object Mode* click on the
# down-pointing arrow of the *Shrinkwrap* modifier and click on *Apply*.
#
