...
class BaseRaw(ProjMixin, ContainsMixin, UpdateChannelsMixin, SetChannelsMixin,
    ...
    def get_data(self, picks=None, start=0, stop=None,
        ...
        units : str | dict | None
            ...
            :ref:`units` and :term:`data channels`). If a string, must be an SI
            unit that will be used to scale the data from all channels of the
            type associated with that unit. This only works if the data
            contains one channel type that has a unit (unitless channel types
            are left unchanged). For example if there are only EEG and STIM
            channels, ``units='uV'`` will scale EEG channels to micro-Volts
            while STIM channels will be unchanged. Finally, if a dictionary is
            provided, keys must be channel types, and values must be units to
            scale the data of that channel type to. For example
