...
.. glossary::
    ...
    equivalent current dipole
        The macroscopic electric potential and magnetic fields measured with
        EEG and MEG, respectively, are generated by electrochemical events at
        the post-synaptic membranes of cortical pyramidal neurons (PNs). Due
        to the anatomical organisation of neocortex, in which the apical
        dendrites of PNs are aligned and perpendicular to the cortical
        surface, the microscopic intracellular currents flowing in thousands
        of cells sum up constructively. The direction of net intracellular
        current is perpendicular to the cortical surface. Since the distance
        of measurement in extra-cranial techniques is 1-2 orders of magnitude
        larger than the characteristic length of intracellular currents
        (100's of microns), the M/EEG field patterns generated by neocortical
        synaptic sources are *equivalent* to those generated by a mathematical
        idealisation of a point-like current source: a current dipole. The
        term Equivalent Current Dipole (ECD), or just "dipole", thus refers
        to the *source model* of a patch of cortex, in which cells are
        experiencing post-synaptic activity (mainly excitation).
