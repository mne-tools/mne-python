...
'dev_head_t', 'line_freq', 'acq_stim', 'proj_id', 'description', 'highpass',
'comps', 'experimenter', 'file_id', 'filenames', 'proj_name',
...
def read_raw_egi(input_fname, event_id):
...
class _RawEDF(Raw):
    ...
    def __init__(self, input_fname, event_ids=None):
        ...
        new_trigger = _combine_triggers(egi_info,
                                        ...
                                        remapping=event_ids)
        self._data = np.concatenate([data, new_trigger])
        self.info = info = Info(dict((k, None) for k _other_fields))
        ...
        my_timestamp = time.mktime(my_timestamp.timetuple())
