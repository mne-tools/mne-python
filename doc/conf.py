...
sphinx_gallery_conf = {
    ...
    "first_notebook_cell": (
        ...
        "# route SourceEstimate.plot() through pyvista-js (vtk.js) instead. This\n"
        "# renders a static activation map on the inflated surface (no time\n"
        "# slider or curvature shading yet). Fully guarded — any failure prints a\n"
        "# message and returns None so the notebook still completes. Only fires\n"
        "# for notebooks that explicitly call it in an emscripten branch.\n"
        ...
        "            _src_vals = _hdata[:, _ti]\n"
        "            # nearest-source fill so the whole surface is colored\n"
        "            _tree = _KDTree(_rr[_vno])\n"
        "            _, _nn = _tree.query(_rr)\n"
        "            _scalars = _src_vals[_nn].astype(float)\n"
        "            # offset hemispheres along x so they do not overlap\n"
        "            _off = -60.0 if _h == 'lh' else 60.0\n"
        "            _pts = _np.round(_rr, 2)\n"
        "            _pts[:, 0] = _pts[:, 0] + _off\n"
        "            _flat = _np.hstack([\n"
        "                _np.full((len(_tris), 1), 3, dtype=_np.int64),\n"
        "                _tris.astype(_np.int64)]).ravel()\n"
        "            _mesh = _pv.PolyData(points=_pts, faces=_flat)\n"
        "            _mesh.point_data['activation'] = _scalars\n"
        "            _plotter.add_mesh(_mesh, scalars='activation',\n"
        "                              cmap='hot', smooth_shading=True)\n"
        "        _plotter.add_scalar_bar(title='activation')\n"
        "        _plotter.show()\n"
        "    except Exception as _e:\n"
