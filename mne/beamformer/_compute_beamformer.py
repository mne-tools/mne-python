...
# @profile
def _normalized_weights(W, G, Cm_inv_sq, reduce_rank, nn, sk,
    ...
    """
    Wk = W.reshape(n_sources, n_orient, -1)
    GkT = G.T.reshape(n_sources, n_orient, -1)
    Gk = GkT.transpose(0, 2, 1)
    tmp = np.matmul(Cm_inv_sq, G)
    tmp = tmp.reshape(-1, n_orient, n_sources, order='F').transpose(2, 0, 1)
    norm_inv = np.matmul(GkT, tmp)
    ...
    if reduce_rank:
        ...
        e, v = np.linalg.eigh(norm_inv)
        e = e[:, np.newaxis, 1:2]
        v = v[:, :, 1:2]
        norm = (v / e) @ v.transpose(0, 2, 1)
        power = np.matmul(norm, np.matmul(Wk, Gk))
    else:
        # Use straight inverse with full rank leadfield
        try:
            # can use solve here; dmalt
            # norm = _repeated_inv(norm_inv, inv_lwork)
            power = np.linalg.solve(norm_inv, np.matmul(Wk, Gk))
        except LinAlgError:
            raise ValueError(
    ...
    max_eval_ind = eig_vals.argmax(axis=1)
    max_power_ori = np.vstack(
        [eig_vecs[i, :, max_eval_ind[i]] for i in range(len(eig_vals))]
    )
    ...
    W = np.matmul(max_power_ori[:, np.newaxis, :], W).squeeze()
    # --------------------------------------------------------- #
    ...
    if (inversion == 'matrix' and pick_ori == 'max-power' and
        ...
        if Wk.ndim == 2 and len(Wk) > 1:
            ...
            if inversion == 'single':
                ...
                # inversion.
                assert Ck.shape == (3, 3)
                # reduce_rank doesn't seem to have any effect here; dmalt
        ...
        if pick_ori == 'max-power':
            # If I use 'max-power' with 'single' and 'unit-noise-gain'
            # it looks like it's gonna crash; dmalt
            # Compute the power
            if inversion == 'single' and weight_norm is not None:
                ...
                # the cov matrix.
                power = Wk.dot(Cm).dot(Wk.T)  # btr use Ck for matrix inv
