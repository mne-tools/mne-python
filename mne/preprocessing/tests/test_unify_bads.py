...
from mne.preprocessing.unify_bads import unify_bad_channels
from mne.time_frequency.tests.test_spectrum import _get_inst()

@pytest.mark.parametrize('instance', ('raw', 'epochs', 'evoked', 'raw_spectrum', 'epochs_spectrum'))
...
def test_error_raising(instance, request, evoked):
    ...
    if instance == 'mixed':
        instance = [_get_inst('raw', request, evoked),
...
def test_bads_compilation(raw):
    '''Tests that bads are compiled properly in two cases: a) single instance
    passed to function with an existing bad, and b) multiple instances passed
    to function with varying compilation scenarios including empty bads,
    unique bads, partially duplicated bads listed out-of-order, and
    nonsense channel names.'''
    ## check unification scenarios
    chns = raw.info['ch_names'][0:3]
    # scnario 1: single instance passed with actual bads (already tested no bads)
