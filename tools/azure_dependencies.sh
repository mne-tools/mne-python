...
if [ "${TEST_MODE}" == "pip" ]; then
	...
	python -m pip install --progress-bar off --upgrade --pre --only-binary ":all:" --no-deps -i --default-timeout=60 "https://pypi.anaconda.org/scipy-wheels-nightly/simple" pandas scikit-learn dipy statsmodels
