...
sphinx_gallery_conf = {
    ...
    "first_notebook_cell": (
        ...
        "# pyvista-js (0.15) has no scalar colormap in its renderer yet, so we\n"
        "# draw a gray base brain plus solid-colored overlays (orange/red) for\n"
        "# the supra-threshold activation, with even scene lighting. Static, one\n"
        "# time point, no time slider yet. Fully guarded — any failure prints a\n"
        "# message and returns None so the notebook still completes.\n"
        ...
        "            if len(_vno) == 0:\n"
        "                continue\n"
        "            _rel = ('MNE-sample-data/subjects/' + _subj +\n"
        "                    '/surf/' + _h + '.inflated')\n"
        "            _lite_fetch_rel(_rel)\n"
        "            _rr, _tris = mne.read_surface(\n"
        "                _sdir + '/' + _subj + '/surf/' + _h + '.inflated')\n"
        ...
        "            _cen = _pts.mean(0)\n"
        "            # gray base brain\n"
        "            _base = _pv.PolyData(points=_pts, faces=_flat(_tris))\n"
        "            _plotter.add_mesh(_base, color=(0.6, 0.6, 0.6),\n"
        "                              smooth_shading=True)\n"
        "            # activation bands as solid-colored overlays, lifted 2%\n"
        "            # off the surface to avoid z-fighting\n"
        "            _fs = _scal[_tris].mean(1)\n"
        "            _p96 = _np.percentile(_scal, 96.0)\n"
        "            _p99 = _np.percentile(_scal, 99.0)\n"
        "            for _lo, _hiv, _col in ((_p96, _p99, (1.0, 0.6, 0.0)),\n"
        "                                    (_p99, None, (0.9, 0.1, 0.1))):\n"
        "                if _hiv is None:\n"
        "                    _fm = _fs >= _lo\n"
        "                else:\n"
        "                    _fm = (_fs >= _lo) & (_fs < _hiv)\n"
        "                if int(_fm.sum()) == 0:\n"
        "                    continue\n"
        "                _uniq, _iv = _np.unique(_tris[_fm], return_inverse=True)\n"
        "                _opts = _cen + (_pts[_uniq] - _cen) * 1.02\n"
        "                _ovl = _pv.PolyData(points=_opts,\n"
        "                                    faces=_flat(_iv.reshape(-1, 3)))\n"
        "                _plotter.add_mesh(_ovl, color=_col,\n"
        "                                  smooth_shading=True)\n"
        "        _plotter.show()\n"
        "    except Exception as _e:\n"
