...
def test_simulate_raw():
    ...
    raw_sim_2 = simulate_raw(info, stc, trans_fname, src_fname, bem_fname,
                             raw_times, random_state=42)
    assert_array_almost_equal(raw_sim_2._data, raw_sim._data)
    ...
    # # TODO: Eventually, add ECG channels. Testing data raw file doesn't
    # #       contain ECG channels yet.
    ...
    raw_sim_2.save(test_outname)

    raw_sim_loaded = Raw(test_outname, preload=True, proj=False,
                         allow_maxshield=True)

    # Some numerical imprecision since save uses 'single' fmt
    assert_allclose(raw_sim_loaded._data[:, :], raw_sim_2._data[:, :],
