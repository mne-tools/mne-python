...
def psd_array_welch(
    ...
    *,
    ...
    if nan_present and aligned_nan:
        ...
        # and SciPy is what does the zero-padding: ``_fft_helper`` calls the FFT
        # with ``n=nfft``, which pads each nperseg-long segment out to nfft. So a
        # span analyzed with a shrunk window still lands on the same frequency grid
        # as the long spans, and simply gets coarser spectral resolution. A per-span
