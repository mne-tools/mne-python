...
from mne.preprocessing import regress, create_eog_epochs
...
def test_regress():
    ...
    epochs_clean, betas = regress(epochs)
    regress(epochs, betas=betas, copy=False)  # inplace, and with betas
    ...
    with pytest.raises(ValueError, match=r'Invalid value.*betas\.shape.*'):
        regress(epochs, betas=betas[:-1])
    with pytest.raises(ValueError, match='cannot be contained in'):
        regress(epochs, picks='eog', picks_ref='eog')
