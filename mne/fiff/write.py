...
def write_double_matrix(fid, kind, mat):
    ...
    data_size = 8 * mat.size + 8 * (mat.ndim + 1)

    fid.write(np.array(kind, dtype='>i8').tostring())
    fid.write(np.array(FIFFT_MATRIX_DOUBLE, dtype='>i8').tostring())
    fid.write(np.array(data_size, dtype='>i8').tostring())
    fid.write(np.array(FIFF.FIFFV_NEXT_SEQ, dtype='>i8').tostring())
    fid.write(np.array(mat, dtype='>f8').tostring())

    dims = np.empty(mat.ndim + 1, dtype=np.int64)
    dims[:mat.ndim] = mat.shape[::-1]
    dims[-1] = mat.ndim
    fid.write(np.array(dims, dtype='>i8').tostring())
