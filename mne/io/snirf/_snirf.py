...
def _get_dataunit_scaling(unit):
    """Return scaling to convert SNIRF dataUnit to MNE base units.

    MNE base units: mol/L for hemoglobin, V for amplitude/voltage.
    """
    if unit == "":
        return 1.0
    ...
    if "mol/" in unit:
        prefix, denom = unit.split("mol/", 1)
        if prefix in _SI_PREFIXES and denom in _VOLUME_IN_LITERS:
            return _SI_PREFIXES[prefix] / _VOLUME_IN_LITERS[denom]

    # Voltage: [prefix]V
    if unit.endswith("V") and unit[:-1] in _SI_PREFIXES:
        return _SI_PREFIXES[unit[:-1]]

    raise RuntimeError(
        f"The data unit {repr(unit)} is not supported "
        "by MNE. Please report this error as a GitHub "
        "issue to inform the developers."
    )
