...
def phase_amplitude_coupling(data, fs, fp_low, fp_high,
                             fa_low, fa_high, bin_size_angle=18):
    ...
    x_fp = band_pass_filter(data, fs, fp_low, fp_high, method='fft', n_jobs=1)
    x_fa = band_pass_filter(data, fs, fa_low, fa_high, method='fft', n_jobs=1)
...
def plot_phase_amplitude_plot(phase_bins, normalized_amplitude):
    ...
    for i in range(len(phase_bins) - 1):  # the bins are always more
        pl.bar(np.rad2deg(phase_bins[i]), normalized_amplitude[i],
