...
if [ "${TEST_MODE}" == "pip" ]; then
	...
	echo "Numpy etc."
	python -m pip install $STD_ARGS --only-binary ":all:" --default-timeout=60 --index-url "https://pypi.anaconda.org/scientific-python-nightly/simple" "numpy>=2.1.0.dev0" h5py
