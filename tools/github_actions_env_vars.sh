...
if [[ "$MNE_CI_KIND" == "pip"* ]] || [[ "$MNE_CI_KIND" == "minimal" ]]; then
    ...
    if [[ "$MNE_CI_KIND" == "pip-pre" ]]; then
        ...
        echo "MNE_TEST_ALLOW_SKIP=.*(Requires (spm|brainstorm) dataset|CUDA not|Numba not|PySide6 causes segfaults|SCIPY_ARRAY_API).*" | tee -a $GITHUB_ENV
    elif [[ "$MNE_CI_KIND" != "pip-ft" ]]; then
        echo "MNE_QT_BACKEND=PySide6" | tee -a $GITHUB_ENV
        echo "MNE_TEST_ALLOW_SKIP=.*(Requires (spm|brainstorm) dataset|pyvista|dipy|pytest-qt|pyqtgraph|nest_asyncio2|pymef|mffpy|CUDA|Freesurfer|MNE-C|OpenMEEG|dataset|SCIPY_ARRAY_API).*" | tee -a $GITHUB_ENV
    elif [[ "$MNE_CI_KIND" == "pip" ]]; then
        echo "MNE_TEST_ALLOW_SKIP=.*(SCIPY_ARRAY_API).*" | tee -a $GITHUB_ENV
    fi
    ...
    echo "MNE_TEST_ALLOW_SKIP=.*(Requires (spm|brainstorm) dataset|CUDA not|Accelerate|Flakey verbose behavior|PySide6 causes segfaults|SCIPY_ARRAY_API).*" | tee -a $GITHUB_ENV
