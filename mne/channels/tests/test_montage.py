...
def test_bvct_dig_montage():
	...
	'<CapTrakFileVersion>1.10</CapTrakFileVersion>\n'
		...
		'<CapTrakElectrodeList>\n'
			...
			'<CapTrakElectrode>\n'
				'<Name>FCz</Name>\n'
				'<X>8.476085349972422</X>\n'
				'<Y>45.851566934840669</Y>\n'
				'<Z>129.44338510687683</Z>\n'
				'<Theta>19.810163497924805</Theta>\n'
				'<Phi>79.526580810546875</Phi>\n'
				'<Radius>137.58561029834266</Radius>\n'
				'<Channel>0</Channel>\n'
			'</CapTrakElectrode>\n'
		'</CapTrakElectrodeList>\n'
		'<ExcludedElectrodes>\n'
	...
	fname = op.join(tempdir, 'test.bvct')
	with open(fname, 'w') as fid:
		fid.write(text)
	dig_montage = read_dig_montage(**{key:fname})
	_check_roundtrip(dig_montage, fname)
	...
	# Test accuracy and embedding within raw object
	raw_bv = read_raw_brainvision(bv_fname, preload=True)
	raw_bv = raw_bv.copy().pick_channels(['FCz'])
	raw_bv.set_montage(dig_montage)

	locs = {'FCz':np.array([0.00847609, 0.04585157, 0.12944339, 0., 0.,0.,
						    np.nan, np.nan, np.nan, np.nan, np.nan, np.nan])}
	for ch_raw, ch in zip(raw_bv.info['chs'],locs):
		assert_equal(ch_raw['ch_name'], ch)
		assert_equal(ch_raw['coord_frame'], FIFF.FIFFV_COORD_HEAD)
		assert_allclose(ch_raw['loc'], locs['FCz'], atol=1e-7)	
