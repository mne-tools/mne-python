...
def _nlxheader_to_dict(matdict: Dict) -> Dict:
    ...
    entries = matdict["Header"][1::, :]  # skip the first row which is just the "Header" string

    return {arr.item().item().split(" ")[0].strip("-"): arr.item().item().split(" ")[-1] 
            for arr in entries if arr[0].size > 0
            }
...
def read_nlx_mat_chan(matfile: str) -> np.ndarray:
    ...
    x = np.concatenate([mat["Samples"][0:n, i] 
                        for i, n in enumerate(n_valid_samples)
                        ]
    ) # in ADBits
...
def test_neuralynx():
    ...
    # ==== MNE-Python ==== #
    raw = read_raw_neuralynx(fname=testing_path, preload=True, exclude_fname_patterns=["*u*.ncs"])

    # test that channel selection worked
    assert raw.ch_names == expected_chan_names, "labels in raw.ch_names don't match expected channel names"
    ...
    mat_y = np.stack([read_nlx_mat_chan(os.path.join(testing_path, ch)) for ch in matchans])


    # ===== Check sample values across MNE-Python, NeuralynxIO and MATLAB ===== #
    assert nlx_ch_names == raw.ch_names # check channel names

    assert_allclose(mne_y, nlx_y, rtol=1e-6, err_msg="MNE and NeuralynxIO not all close")           # data
    assert_allclose(mne_y, mat_y, rtol=1e-6, err_msg="MNE and Nlx2MatCSC.m not all close")          # data

    _test_raw_reader(read_raw_neuralynx, fname=testing_path, exclude_fname_patterns=["*u*.ncs"])
