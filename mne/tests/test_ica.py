...
test_cov_name = op.join(op.dirname(__file__), '..', 'fiff', 'tests', 'data',
                     'test-cov.fif')
...
def test_decompose_raw():
    """ Test raw decomposition
    """
    ica.decompose_raw(raw, picks=picks)
    ica_cov.decompose_raw(raw, picks=picks)


def test_get_sources_raw():
    """ Test raw source-extraction
    """
    sources = ica.get_sources_raw(raw, picks=picks)
    assert sources.shape[0] == ica.n_components

    ica_cov.get_sources_raw(raw, picks=picks)
    assert sources.shape[0] == ica.n_components


def test_pick_sources_raw():
...
def test_get_sources_epochs():
    """ Test epochs sources selection
    """
    sources = ica.get_sources_epochs(epochs, picks=picks)
    assert sources.shape[1] == ica.n_components

    sources = ica_cov.get_sources_epochs(epochs, picks=picks)
    assert sources.shape[1] == ica.n_components


def test_pick_sources_epochs():
    """ Test sources selection
    """
    epochs2 = ica.pick_sources_epochs(epochs, exclude=[],
                                      sort_method='kurtosis', copy=True)
