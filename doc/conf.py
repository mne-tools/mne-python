...
# more than it saves. Swap just that cell for a note that keeps the code in
# view and says what it needs instead. Keep this list short: a page that is
# mostly unavailable belongs in JUPYTERLITE_EXCLUDE rather than here.
JUPYTERLITE_CELL_NOTES = (
    (
...
def _lite_note_unrunnable_cells(notebook_content, notebook_filename):
    ...
    contents, so only the browser copy changes -- the notebook offered for
    download stays exactly as the docs built it.
    """
    path = str(notebook_filename).replace(os.sep, "/")
    for suffix, needle, note in JUPYTERLITE_CELL_NOTES:
        ...
        for cell in notebook_content.get("cells", []):
            ...
            sphinx_logger.info(f"[JupyterLite]   {suffix}: noted {needle} cell")
...
sphinx_gallery_conf = {
    "jupyterlite": {
        ...
        "jupyterlite_contents": "jupyterlite_contents",
        "notebook_modification_function": _lite_note_unrunnable_cells,
    },
    "first_notebook_cell": (
