...
def test_compute_proj_eog_multiple():
    """Test computation of ExG SSP projectors with multiple Raw"""
    average = False
    raw = Raw(raw_fname, preload=True)
    projs, events = compute_proj_ecg(raw, n_mag=2, n_grad=2, n_eeg=2,
                                         ch_name='MEG 1531', bads=['MEG 2443'],
    ...
    raw = [Raw(raw_fname, preload=True), Raw(raw_fname, preload=True)]
    projs_2, events = compute_proj_ecg(raw, n_mag=2, n_grad=2, n_eeg=2,
                                         ch_name='MEG 1531', bads=['MEG 2443'],
                                         average=average, avg_ref=True,
                                         no_proj=True)
    raw[0].close()
    ...
    raw = [Raw(raw_fname, preload=True), Raw(raw_fname, preload=False)]
    assert_raises(ValueError, compute_proj_eog, raw)
    raw[0].close()
    raw[1].close()
