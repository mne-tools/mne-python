...
def _trace_click_xy(fig, trace_idx, sample_idx):
    ...
    The pyqtgraph browser applies a per-trace y-scaling transform that
    ``get_ydata()`` does not reflect, so clicking ``(x, get_ydata()[i])`` can
    miss the rendered curve. Ask pyqtgraph for the trace's actual rendered
    position and map it back to data coordinates. The matplotlib browser has
    no such transform, so ``get_ydata()`` is already the clickable location.
    """
