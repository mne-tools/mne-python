...
from mne.forward._lead_dots import (_comp_sum_eeg, _get_legen_table,
                                    _get_legen_lut)
...
def test_legendre_val():
    ...
    # Table approximation
    lut, n_fact = _get_legen_table('eeg')
    # Need a "1:" here because we omit the first coefficient in our table!
    vals_i = _get_legen_lut(xs, lut)
    assert_allclose(vals_np[:, 1:], vals_i, rtol=1e-2, atol=1e-3)

    # Now let's look at our sums
    ctheta = np.random.rand(20, 30) * 2.0 - 1.0
    beta = np.random.rand(20, 30) * 0.8
    c1 = _comp_sum_eeg(beta.flatten(), ctheta.flatten(), lut, n_fact)
    c1.shape = beta.shape
