...
def _make_scorer(scoring):
    ...
    scoring : str | callable
        If str, must be compatible with sklearn make_scorer.
        If callable, must be of form scoring(y_true, y_pred, *kwargs)
    """
    ...
    if not isinstance(scoring, str):
        return make_scorer(scoring) if scoring is not None else scoring

    elif scoring is not None:
        return get_scorer(scoring)
    else:
        raise ValueError(
            'scoring must be str or scorer got %s instead' % type(scoring))
