...
###############################################################################
# Information about the channels contained in the `Raw` object is contained in
# the `info` attribute. This is essentially a dictionary with a number of
# relevant fields (see :ref:`tut_info_objects`).
#
# Indexing data
# ------------
# There are two ways to access the data stored within `Raw` objects. One is by
# accessing the underlying data array, and the other is to index the `Raw`
# object directly.
#
# To access the data array of `Raw` objects, use the `_data` attribute. Note
# that this is only present if `preload==True`.
...
###############################################################################
# You can also pass an index directly to the `Raw` object. This will return an
# array of times, as well as the data representing those timepoints. This may
# be used even if the data is not preloaded:
...
# ---------------------------
#
# `Raw` objects can be concatenated in time by using the `append` function. For
# this to work, they must have the same number of channels and their `info`
