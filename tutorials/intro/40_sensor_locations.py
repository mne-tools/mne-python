...
# positions in 3D (``x``, ``y``, ``z``, in meters), which can be assigned to
# existing EEG/MEG data. By specifying the location of sensors relative to the
# brain, :class:`Montages <mne.channels.DigMontage>` play an important role in
# computing the forward solution and inverse estimates.
#
# In contrast, :class:`Layouts <mne.channels.Layout>` are *idealized* 2D
...
# :func:`mne.channels.make_standard_montage` (note that you need to provide the
...
# :meth:`~mne.io.Raw.set_montage` method, for example
# :meth:`raw.set_montage <mne.io.Raw.set_montage>`. It is also possible to skip
# the loading step by passing the filename string directly to the
# :meth:`~mne.io.Raw.set_montage` method. This will not work with our sample
...
# :meth:`~mne.channels.DigMontage.plot` method for visualizing the sensor
# locations in 2D or 3D:

ten_twenty_montage.plot(kind='3d').gca().view_init(azim=70, elev=15)
...
# positions on a sphere. Because the ``'standard_1020'`` montage
# contains realistic (as opposed to idealized spherical) channel positions, we
# will use a different montage to demonstrate how channels are projected to 2D:
...
# that plots channels in 2D (like :meth:`~mne.channels.DigMontage.plot` that we
# use here, but also for example :func:`mne.viz.plot_topomap`):
...
# ``info`` attribute of the :class:`~mne.io.Raw` object (see the documentation
# of the reading functions and :meth:`~mne.io.Raw.set_montage` for details on
# how that works). Therefore, we can plot sensor locations directly from the
# :class:`~mne.io.Raw` object using :meth:`~mne.io.Raw.plot_sensors`, which
# provides similar functionality to
# :meth:`montage.plot() <mne.channels.DigMontage.plot>`. In addition,
# :meth:`~mne.io.Raw.plot_sensors` supports channel selection by type,
# color-coding channels in various ways (by default, channels listed in
# ``raw.info['bads']`` will be plotted in red), and drawing in an
# existing matplotlib ``Axes`` object (so the channel positions can easily be
# added as a subplot in a multi-panel figure):
...
# The resulting :class:`montage <mne.channels.DigMontage>` can then be added to
# :class:`~mne.io.Raw` objects by passing it as an argument to the
...
# plotted with the :meth:`~mne.channels.DigMontage.plot` and saved with the
# :meth:`~mne.channels.DigMontage.save` methods of the
# :class:`montage <mne.channels.DigMontage>` object.
#
# .. note::
#
#     When setting a montage with :meth:`~mne.io.Raw.set_montage`,
#     the measurement info is updated in two places (both ``chs``
#     and ``dig`` entries are updated) – see :ref:`tut-info-class` for more
#     details. Note that ``dig`` may contain HPI, fiducial, or head shape
#     points in addition to electrode locations.
#
#
# Rendering sensor position with Mayavi
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
# It is also possible to render an image of an MEG sensor helmet in 3D using
# Mayavi instead of matplotlib. This works by calling
...
###############################################################################
# Note that :func:`~mne.viz.plot_alignment` requires an :class:`~mne.Info`
# object, and can also render MRI surfaces of the scalp, skull, and brain (by
# passing a dict with keys like ``'head'``, ``'outer_skull'`` or ``'brain'`` to
# the ``surfaces`` parameter). This makes the function useful for
...
# passing the layout to :func:`mne.viz.plot_layout`:
...
###############################################################################
# If you have a :class:`~mne.io.Raw` object that contains sensor positions, you
# can create a :class:`~mne.channels.Layout` object with either
...
#     :func:`mne.channels.find_layout` with the ``ch_type`` parameter
#     (as previously demonstrated for EEG).
#
# All :class:`~mne.channels.Layout` objects have a
# :meth:`~mne.channels.Layout.save` method that writes layouts to disk as
# either :file:`.lout` or :file:`.lay` formats (inferred from the file
# extension contained in the ``fname`` argument). The choice between
# :file:`.lout` and :file:`.lay` format only matters if you need to load the
# layout file in some other application (MNE-Python can read both formats).
#
#
# .. LINKS
#
