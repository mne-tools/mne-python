...
def test_resolution_metrics_surface():
    ...
    fwd = mne.convert_forward_solution(fwd, surf_ori=True, force_fixed=True, copy=False)
    ...
    rm_mne = make_inverse_resolution_matrix(fwd, inv, method="MNE", lambda2=lambda2)
    # compute very smooth MNE
    rm_mne_smooth = make_inverse_resolution_matrix(
        fwd, inv, method="MNE", lambda2=100.0
    )
    # compute resolution matrix for sLORETA
    rm_lor = make_inverse_resolution_matrix(fwd, inv, method="sLORETA", lambda2=lambda2)

    # Compute localisation error (STCs)
    # Peak
    le_mne_psf = resolution_metrics(
        rm_mne, fwd["src"], function="psf", metric="peak_err"
    )
    le_mne_ctf = resolution_metrics(
        rm_mne, fwd["src"], function="ctf", metric="peak_err"
    )
    le_lor_psf = resolution_metrics(
        rm_lor, fwd["src"], function="psf", metric="peak_err"
    )
    # Centre-of-gravity
    cog_mne_psf = resolution_metrics(
        rm_mne, fwd["src"], function="psf", metric="cog_err"
    )
    cog_mne_ctf = resolution_metrics(
        rm_mne, fwd["src"], function="ctf", metric="cog_err"
    )

    # Compute spatial spread (STCs)
    # Spatial deviation
    sd_mne_psf = resolution_metrics(rm_mne, fwd["src"], function="psf", metric="sd_ext")
    sd_mne_psf_smooth = resolution_metrics(
        rm_mne_smooth, fwd["src"], function="psf", metric="sd_ext"
    )
    sd_mne_ctf = resolution_metrics(rm_mne, fwd["src"], function="ctf", metric="sd_ext")
    sd_lor_ctf = resolution_metrics(rm_lor, fwd["src"], function="ctf", metric="sd_ext")
    # Maximum radius
    mr_mne_psf = resolution_metrics(
        rm_mne, fwd["src"], function="psf", metric="maxrad_ext", threshold=0.6
    )
    mr_mne_psf_smooth = resolution_metrics(
        rm_mne_smooth, fwd["src"], function="psf", metric="maxrad_ext", threshold=0.6
    )
    mr_mne_ctf = resolution_metrics(
        rm_mne, fwd["src"], function="ctf", metric="maxrad_ext", threshold=0.6
    )
    mr_lor_ctf = resolution_metrics(
        rm_lor, fwd["src"], function="ctf", metric="maxrad_ext", threshold=0.6
    )
    # lower threshold -> larger spatial extent
    mr_mne_psf_0 = resolution_metrics(
        rm_mne, fwd["src"], function="psf", metric="maxrad_ext", threshold=0.0
    )
    mr_mne_psf_9 = resolution_metrics(
        rm_mne, fwd["src"], function="psf", metric="maxrad_ext", threshold=0.9
    )

    # Compute relative amplitude (STCs)
    ra_mne_psf = resolution_metrics(
        rm_mne, fwd["src"], function="psf", metric="peak_amp"
    )
    ra_mne_ctf = resolution_metrics(
        rm_mne, fwd["src"], function="ctf", metric="peak_amp"
    )
    ...
    with pytest.raises(ValueError, match="a recognised resolution function"):
        resolution_metrics(rm_mne, fwd["src"], function="foo", metric="peak_err")
    ...
    assert_((le_lor_psf.data == 0.0).all())
