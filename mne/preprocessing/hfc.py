...
def apply_hfc(raw, order=1, exclude_bads=True, verbose=None):
    ...
    explained as a homogenous or harmonic field. Useful for arrays of OPMs.
    ...
    order: int 
        ...
        only the homogenous field component (default), 2 to add gradients, 3 
...
class FieldCorrector():
    """Constructor for homogenous/harmonic field correction, contains all
    assests required for offline subtraction of the modelled environemntal
    inteference, and forward model compensation.

    Parameters
    ----------
    inst : instance of Raw
        The instance to generate model from.
    picks: list of sensor indicies to build model with. Default of None will 
...
def _filter_channels_with_positions(info, indsin):
    """Keep indicies of channels with poisition information."""
    ch_inds = list()
    for ii in indsin:
