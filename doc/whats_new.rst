...
- The functions ``lcmv``, ``lcmv_epochs``, and ``lcmv_raw`` are now deprecated in favor of ``make_lcmv`` and ``apply_lcmv``, ``apply_lcmv_epochs``, and ``apply_lcmv_raw``, by `Britta Westner`_  
