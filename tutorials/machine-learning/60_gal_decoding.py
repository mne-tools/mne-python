...
Traditionally, multi-variate pattern analysis (MVPA) or decoding techniques uses spatial information (features)
at every single time-point to train and test classifiers. However, some electrophysiological 
components are better captured on the time domain than in the spatial one. Therefore, 
this tutorial focus on such temporal information, serving as features to train our 
classifier models. As a result, a topographic representation of decoding rates is 
obtained, depicting the location of brain areas involved in the analized neural component.
...
pattern transfers across the sensor array. The Generalization across Location (GAL) procedure computes 
a channel by channel matrix revealing the generalization between brain areas. Then, a 
connectivity plot can be used to represent the cross-decoding generalization patterns.
This cross-decoding technique is analogous to the temporal generalization by King & Dehaene 
(:ref:`temporal generalization example <tut-mvpa>` ), yet in orthogonal direction. Instead of 
showing the temporal sustainability of a neural representation, it shows the spatial displacement 
...
carried out both at the sensor level (suitable on EEG electrodes) or source level (cortical vertices or ROI).
In the case of using EEG data, it is strongly recommended to apply laplacian correction to avoid
spurious cross-decoding generalization between proximal areas due to volume conductance. In this tutorial 
no Laplacian has been applied into data to keep it short and focus on decoding.
...
# Here, we introduce the space-resolved decoding where information from each sensor is used
# in the analysis, utilizing all the time points as features. We use the SlidingEstimator function
# and the outcome can be display using a topographic map.
# Importantly, :class:`~mne.decoding.GeneralizingEstimator` treats the final dimension (usually time) as
# the iteration axis by default. ``axis=1`` selects sensors instead of time, thus changing from temporal decoding
# to spatial decoding.
...
space_decoder = SlidingEstimator(classifier, scoring="roc_auc", n_jobs=1, verbose=False, axis=1)
...
# --------------------------------------------
# Training the classifier model in one sensor and estimating its decoding accuracy in every other 
# results in the creation of the GAL matrix. This sensor by sensor matrix shows the capability of 
# one sensor or brain area to classify the same differences between conditions in other cortical area.
...
# using SlidingWindow (see the topographic map above). Off-diagonal cells test the temporal
# pattern learned at one location at another location. Significant below-chance transfer
# can occur when the two locations carry opposite-polarity patterns. Importantly, a single
# participant's matrix is not a group-level inference. To know which elements of the matrix are 
# statistically significant, a two-sided contrast against chance level (here 0.5) must be carried 
# out using all the n GAL matrix from all the participants.
...
# Thus, this results can be drawn in a topographic connectivity plot, illustrating which brain 
# areas seem to be showing similar neuronal responses. For simplicity, only the most important 15 lines are shown.
# Blue lines represents those cross-decoding generalization showing similar temporal patterns (significantly above chance-level), 
# while red lines indicates those with opposite temporal patterns (below chance-level).
# Notably, this is a display of sensor-generalization scores based on decoding, not a physical-connectivity
# graph.
...
# ------------------------------------------------------------
# One step further is to estimate the Time-GAL matrix. The Time-GAL procedure pairs the 
# decoding backward model (decoding GAL matrix) with a forward one (correlation matrix), thus revealing the generalization connectivity in space and time.
# The correlation matrix indicates when (time) and where (sensor) differences between conditions occour. 
# Here, we compute Pearson's correlation between the binary face or car label and each sensor and time point
# signal. As a result, a channel by time correlation matrix arise.
...
# ------------------------------------------------------------
# To obtain the Time-GAL information, it needs to pair the GAL decoding information 
# with the information of the correlation matrix. To combine both matrices, first 
# these matrices must be thresholded into significant or relevent information based on its
# distance with the chance-level (GAL) or 0 (Pearson's correlation). 
# Once every subject's matrices are computed, such threshold can be obtaind by finding the statistically significant 
# elements. However, in this single participant tutorial, we select only those elements with 
# values higher to 0.6 or lesser than 0.4 for the GAL matrix, and +- 0.25. The combination results in a 
# sensor by sensor by time 3D matrix where 0s means lack of any relevant information, and values indicate 
# the strenght of the GAL connection between two areas or sensors during specific time points.


relevant_timechannels = np.abs(label_signal_correlation) > 0.25
relevant_GAL = ((mean_scores - 0.5) > 0.1) + -1*((mean_scores - 0.5) < -0.1)

timeGAL = np.zeros((relevant_GAL.shape[0], relevant_GAL.shape[1], relevant_timechannels.shape[1]))
for i in range(relevant_GAL.shape[0]):
    for j in range(relevant_GAL.shape[0]):
        if relevant_GAL[i,j] != 0:
            timeGAL[i,j,:] = (mean_scores[i,j]-0.5) * (relevant_timechannels[i,:] * np.abs(label_signal_correlation[i,:]) )
...
# evolve across time. It depicts the spatio-temporal representation of the inteaction 
...
for k, ax in enumerate(axes):
    ...
    mne.viz.plot_sensors(epochs.info, kind='topomap', show_names=False, axes=ax, show=False)
    pos = ax.collections[0].get_offsets()
    ...
    for i in range(len(epochs.ch_names)):
        for j in range(i + 1, len(epochs.ch_names)):
            ...
            if abs(val) >= thresh and val != 0:
                ...
                ax.plot(pos[[i, j], 0], pos[[i, j], 1], color='red' if val > 0 else 'blue',
                        linewidth=1 + 4 * nw, alpha=0.3 + 0.7 * nw)
...
# --------------------------------------------------------
# The present tutorial illustrates the estimation of spatial decoding, GAL and TimeGAL procedures. 
# However, it only shows the EEG activity of one participant, first of the ERP-Core dataset.
# Therefore, results only depicts the decoding of a single participant and statistical significance 
# cannot be calculated. Ideally, this analysis is computed for every single participant of the experimental sample.
# When the whole group is calculated and its data extracted, the topographic map, the GAL 
# matrix and the Pearson's correlation matrix from each participant can be used as distribution for a pair 
# test such as Student's T to measure the statistically differences with the chance-level or 0, respectively. 
# Notably, these statistical analysis must meet a family-wise error (FWER) correction such as 
# permutation test or Bonferroni, thus ensuring that the decoding results are accurate and robust.
#
